...
async def on_command(message: Message) -> None:
    ...
    if guild_player.repeat_queue:
        ...
        await utils.MessageUtil.reply_fancy_message(":white_check_mark: 關閉: 重複播放清單", discord.Colour.green(), message)
    else:
        guild_player.repeat_queue = True
        await utils.MessageUtil.reply_fancy_message(":white_check_mark: 開啟: 重複播放清單", discord.Colour.green(), message)