...
class PlayCommand(WDCommand):
    def __init__(self, commandsManager: CommandsManager) -> None:
        super().__init__(commandsManager, "add", [], category="控制類")


@main_command("插播一首歌。下一首歌就是1，取代這首就是0，下兩首就是2，依此類推", PlayCommand, index="位置", song="影片連結")
async def on_command(message: Message, index: int, *, song: str) -> None:
    ...
    try:
        ...
        for video in playlist.videos:
            ...
            await guild_player.insert_to_queue(await guild_player.get_track_from_youtube_pytube(video), index)
    ...
    if await guild_player.insert_to_queue(track, index):
        embed = discord.Embed()
